...
1. 使用 svg 自動縮放，線框很明顯與範例有落差，原因在於在相同的 `viewBox` 下，`stroke-width` 也會跟著縮放，並不會像一般元素的 `border-width: 2px` 會固定在某個像素。

2. 切換數量後，新增出來的矩形動畫，會與原本矩形動畫不同步，原因在於原本在畫面上的矩形動畫，不會因為新增了其他的矩形動畫而重新執行動畫，而是持續進行自己的循環。
